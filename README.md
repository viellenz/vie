# vie.lcf

`vie.lcf` 是一份 Loon 远端配置文件，基于可莉 Loon 进阶配置整理，保留原配置的 DNS、防泄漏、MITM、复写、插件与基础直连规则，并补充常用服务分流策略。

正式入口只有一个：

```text
https://raw.githubusercontent.com/viellenz/vie/main/vie.lcf
```

仓库版不包含机场订阅、本地节点、MITM 证书、私有参数。节点订阅和本地节点应在 Loon 内单独添加。

## 配置摘要

| 项目 | 当前值 |
| --- | --- |
| 文件 | `vie.lcf` |
| 配置版本 | `2026-06-10-short-policy-v7` |
| 最低 Loon 版本 | `3.2.3 (754)`，由 `ip-mode`、`ipasn-url` 要求 |
| Loon 模式 | 分流配置 |
| 主策略 | `Proxies` |
| 兜底规则 | `FINAL,Proxies` |
| IP 模式 | `ipv4-only` |
| 策略切换 | `disconnect-on-policy-change = true` |
| 可见策略组 | 18 个 |
| 订阅节点 | 不写入仓库版 |

## 导入方式

在 Loon 中导入上面的正式入口地址，然后确认：

1. 添加机场订阅或本地节点。
2. 更新远端资源。
3. 进入策略页，确认 `Proxies` 中能看到节点。
4. 选择 `Proxies` 内的具体节点。
5. 打开请求记录或 IP 检测页面确认出口变化。

## 策略组

`[Proxy Group]` 共 18 个可见策略组，显示顺序与下表一致：

| # | 策略组 | 用途 | 默认候选 |
| --- | --- | --- | --- |
| 1 | `Proxies` | 全局代理入口 | `全球节点` |
| 2 | `Telegram` | Telegram 分流 | `Proxies`、地区组、`其余节点`、`DIRECT` |
| 3 | `Google` | Google 分流 | `Proxies`、地区组、`其余节点`、`DIRECT` |
| 4 | `Gemini` | Gemini 分流 | `Google`、`Proxies`、地区组、`其余节点` |
| 5 | `AI` | AI 服务分流 | `Proxies`、地区组、`其余节点` |
| 6 | `TikTok` | TikTok 分流 | `Proxies`、地区组、`其余节点` |
| 7 | `Social` | 社交服务分流 | `Proxies`、地区组、`其余节点`、`DIRECT` |
| 8 | `YouTube` | YouTube 分流 | `Proxies`、地区组、`其余节点`、`DIRECT` |
| 9 | `Spotify` | Spotify 分流 | `Proxies`、地区组、`其余节点` |
| 10 | `Netflix` | Netflix 分流 | `Proxies`、地区组、`其余节点` |
| 11 | `Microsoft` | Microsoft 分流 | `DIRECT`、`Proxies`、地区组、`其余节点` |
| 12 | `Apple` | Apple 分流 | `DIRECT`、`Proxies`、地区组、`其余节点` |
| 13-18 | `US` `HK` `JP` `TW` `SG` `KR` | 地区内自动测速 | 对应地区筛选器，`url-test` 自动选择低延迟节点 |

`其余节点` 不是可见策略组，它只存在于 `[Remote Filter]`，供服务策略组选用。`Proxies` 保持手动选择；`US`、`HK`、`JP`、`TW`、`SG`、`KR` 使用 `url-test`，在对应地区内每 600 秒测速一次，`tolerance = 100` 用于减少频繁切换。

## 流量路径

```text
请求
  -> 规则匹配（本地规则 > 插件规则 > 订阅规则，订阅规则按 [Remote Rule] 顺序）
  -> 规则指定策略组
  -> 策略组选中的节点或内置策略
```

`Final` 不是策略组，是 `[Rule]` 中的兜底规则 `FINAL,Proxies`：所有未命中前置规则的请求默认交给 `Proxies`。

## 规则来源

### 规则顺序约束

`[Remote Rule]` 中顺序即优先级：同为订阅规则时，排在前面的列表先匹配。当前顺序经过逐域名遮蔽分析验证，调整前必须重新检查：

```text
Gemini -> YouTube -> Google -> AI -> Telegram -> Spotify -> Netflix
  -> Social(Twitter/Instagram/WhatsApp/Discord/Line/Facebook/Reddit)
  -> TikTok -> Speedtest Intl -> GitHub
  -> Microsoft -> Apple
  -> 国内直连列表(WeChat...DingTalk) -> LAN -> CN REGION
```

约束原因：

- Microsoft.list 含 `azureedge.net`、`windows.net`、`azure.com`、`bing.com`、`edgesuite.net` 等宽后缀，且 Microsoft 策略组默认 DIRECT。它排前面时会把 `copilot.microsoft.com`、`sydney.bing.com`、OpenAI 的 Azure CDN、`netflix.com.edgesuite.net` 等截成直连，所以 Microsoft/Apple 必须排在所有代理类规则之后。
- AI 排在 Google 之后：Google 系 AI 域名（`aistudio.google.com`、`notebooklm.google` 等）留给 Google 组，与 Gemini 保持账号出口一致；非 Google 的 AI 域名由 AI 组接管。
- TikTok 排在 ByteDance 之前：TikTok 解锁依赖 `bytedance.com`、`pstatp.com` 等基建域名走代理，这是可莉规则的有意设计，不要"修正"。
- 本地 `[Rule]` 中的透明代理 fake-IP 直连规则（28.0.0.1/8、198.18.0.0/15）刻意不加 `no-resolve`：依靠域名解析出 fake-IP 来识别透明代理路由器，加了机制就失效。

### blackmatrix7

主要应用分流与国内应用直连规则来自 `blackmatrix7/ios_rule_script` 的 Loon 规则目录：

| 类型 | 策略 |
| --- | --- |
| Gemini | `Gemini` |
| YouTube | `YouTube` |
| Google | `Google` |
| Telegram | `Telegram` |
| Microsoft | `Microsoft` |
| Apple | `Apple` |
| Spotify | `Spotify` |
| Netflix | `Netflix` |
| Twitter / Instagram / WhatsApp / Discord / Line / Facebook / Reddit | `Social` |
| WeChat / Tencent / Alibaba / ByteDance / Baidu / BiliBili 等 | `DIRECT` |

### 可莉规则

以下规则保留可莉规则源或可莉规则镜像：

| 规则 | 策略 | 地址 |
| --- | --- | --- |
| TikTok | `TikTok` | `https://kelee.one/Tool/Loon/Lsr/TikTok.lsr` |
| AI | `AI` | `https://kelee.one/Tool/Loon/Lsr/AI.lsr` |
| Speedtest International | `Proxies` | `https://kelee.one/Tool/Loon/Lsr/SpeedtestInternational.lsr` |
| LAN | `DIRECT` | `https://kelee.one/Tool/Loon/Lsr/LAN_SPLITTER.lsr` |
| CN REGION | `DIRECT` | `https://kelee.one/Tool/Loon/Lsr/REGION_SPLITTER.lsr` |

Netflix 规则源自 v6 起改为 blackmatrix7 直连：实测可莉 Netflix.lsr 与 blackmatrix7 Netflix.list 内容逐字节一致（可莉源即 blackmatrix7 数据的每日转录），直连源头少一个镜像跳板。`git.repcz.link` 镜像仍被 `[Plugin]` 中的 AppleWeatherEnhancer 使用——直连 kelee.one 拉取该插件在部分网络环境会返回 403。

## 图标来源

| 类型 | 来源 |
| --- | --- |
| Proxies | `shindgewongxj/WHATSINStash` |
| Telegram / Apple | `blackmatrix7/ios_rule_script` Qure 图标 |
| Google / YouTube | `Semporia/Hand-Painted-icon` |
| Gemini | `lige47/QuanX-icon-rule` |
| AI | `Orz-3/mini` |
| TikTok / Social / Spotify / Netflix | `luestr/IconResource` |
| Microsoft / 地区图标 | `Koolson/Qure` |

## 插件

| 插件 | 状态 | 用途 |
| --- | --- | --- |
| `Block_HTTPDNS` | 启用 | 拦截常见 HTTPDNS |
| `BlockAdvertisers` | 启用 | 广告平台拦截基础插件 |
| `QuickSearch` | 启用 | Safari 快捷搜索 |
| `Prevent_DNS_Leaks` | 启用 | DNS 防泄漏，策略为 `Proxies` |
| `Node_detection_tool` | 启用 | 节点检测工具 |
| `AppleWeatherEnhancer` | 启用 | Apple 天气增强 |
| `iRingo Maps` | 启用 | 地图增强 |
| `TestFlightRegionUnlock` | 禁用 | TestFlight 区域解锁，需要时手动启用 |
| `BoxJs` | 启用 | 脚本数据管理 |
| `Sub-Store` | 启用 | 订阅管理 |
| `Script-Hub` | 启用 | 脚本转换和管理 |

## Loon 开关建议

建议在 Loon 中开启：复写、脚本、插件、MITM。

MITM 需要安装并信任证书，仓库版不保存证书内容。`[Script]` 为空时 Loon 首页脚本数量显示 `0` 是正常现象，插件内部脚本不一定计入这里。

## 常见问题

### 看不到 Final

正常。`Final` 是 `[Rule]` 中的兜底规则，不是策略组，见「流量路径」。

### 切换节点后出口 IP 不变

优先检查：

1. 是否切换了 `Proxies` 内的具体节点。
2. 当前请求是否命中了某个服务策略组，而该服务策略组选择了地区组；地区组会在对应地区内自动测速选节点。
3. 请求是否复用了旧连接。
4. Loon 是否已更新远端资源。

当前配置启用 `disconnect-on-policy-change = true`：策略切换后旧连接会被断开，新请求应更快使用新节点。

### Google 与 Gemini 分流异常

`Gemini` 默认第一项是 `Google`：

```ini
Gemini = select,Google,Proxies,US,HK,JP,TW,SG,KR,其余节点
```

这样可以让 Gemini 与 Google 登录、验证相关请求尽量保持同一出口。若需要单独指定 Gemini 出口，可在 `Gemini` 策略组中手动切换。

### 国内应用偶发走代理

先查看请求记录命中的规则。如果命中 `FINAL,Proxies`，说明该域名未被当前国内规则覆盖。可以临时补本地规则，或等待上游规则更新。

### Netflix 无法解锁

规则只能把 Netflix 流量导向 `Netflix` 策略组，不能保证节点本身具备解锁能力。排查顺序：

1. 在 `Netflix` 策略组手动选择一个支持 Netflix 的节点。
2. 使用节点检测工具确认 Netflix 状态。
3. 检查 DNS、缓存、App 登录地区和节点原生地区。
4. 重新打开 Netflix App 或清理相关缓存。

### TikTok 无法解锁

TikTok 解锁通常同时受节点地区、系统地区、SIM 状态、DNS、App 缓存影响。规则只负责流量分流。

### QUIC 相关问题

Google、YouTube 等服务可能使用 QUIC，也就是 UDP 443。节点或网络不稳定时，可能出现加载慢、测速异常、请求记录中出现 QUIC 被拦截。

Loon 没有 `block-quic` 参数。当前配置的 `udp-fallback-mode = REJECT` 已覆盖节点不支持 UDP 的场景：QUIC 被拒绝后应用自动回落 TCP。如需主动全局阻断 QUIC，在 `[General]` 中添加 `disable-udp-ports = 443`。

### GitHub Raw 链接延迟

GitHub Raw 可能短时间缓存旧内容。仓库内容已更新但 Raw 仍显示旧版时，等待一段时间或刷新资源即可。

## 文件结构

`vie.lcf` 使用 Loon 标准配置段：

```ini
[General]
[Proxy]
[Remote Proxy]
[Remote Filter]
[Proxy Group]
[Rule]
[Remote Rule]
[Host]
[Rewrite]
[Script]
[Plugin]
[Mitm]
```

结构来源：

- 主体配置、DNS、插件、MITM、复写结构：可莉 Loon 进阶配置。
- 策略组显示结构：短命名结构，使用 `Proxies`、服务组、地区组；`Proxies` 手动选择，地区组使用 `url-test`。
- 规则补充：按服务拆分到独立策略组，节点由 Loon 本地订阅提供。

## 维护规则

- 只维护 `vie.lcf` 一个正式配置文件。
- 不提交机场订阅、本地节点、MITM 证书、个人密钥。
- 新增策略组后必须同步 `[Remote Rule]` 中的 `policy=`。
- 调整 `[Remote Rule]` 顺序前必须检查宽后缀遮蔽：代理类规则在 Microsoft/Apple 之前，TikTok 在 ByteDance 之前，LAN 和 CN REGION 垫底。
- 新增图标后必须验证 URL 可访问。
- 调整策略组顺序后必须重新检查 Loon 策略页显示顺序。
- 修改 README 时必须同步版本号、策略组数量和规则来源。

## 审查清单

每次修改后至少检查：

```text
Proxy Group 数量与顺序
policy= 是否都指向有效策略组或内置策略
策略组成员是否存在于策略组、Remote Filter 或内置策略
Remote Filter 是否全部被策略组引用（无死过滤器）
Remote Rule 顺序符合「规则顺序约束」，宽后缀遮蔽无方向错误
Netflix / AI / TikTok 等远程规则是否只有一条有效入口
FINAL 是否仍为 FINAL,Proxies
README 版本号与 vie.lcf 一致
```

当前已确认：

```text
GROUP_COUNT=18
GROUP_ORDER_CHECK=OK
REMOTE_RULES=40
REMOTE_RULE_ORDER_CHECK=OK
MISSING_POLICY=none
MISSING_MEMBERS=none
DEAD_FILTERS=none
NETFLIX_RULE_COUNT=1
NETFLIX_RULE_CHECK=OK
FINAL=Proxies
SHADOW_FLIP_COUNT=3（仅 TikTok 收编 ByteDance 基建域名，解锁设计，保留）
VERSION=2026-06-10-short-policy-v7
GIT_HISTORY_SECRET_SCAN=clean（全历史 31 个 blob，无订阅/节点/证书/密钥）
```

## 参考资料

- Loon 官方文档：`https://nsloon.app/docs/intro/`
- Loon 策略说明：`https://nsloon.app/docs/Policy/`
- Loon 规则说明：`https://nsloon.app/docs/category/%E8%A7%84%E5%88%99/`
- Loon 插件说明：`https://nsloon.app/docs/Plugin/`
- Loon Scheme / 统一链接：`https://nsloon.app/docs/Scheme/`
- Loon 配置知识库：`https://coffee-elderberry-22b.notion.site/Loon-71747252d5054551a8cd10924064899c`
- Loon 资源与图标知识库：`https://getupnote.com/share/notes/zSn1ShBmzNYISKcTgjXE5oHMrNf2/b6047d8b-621c-44af-bfa6-a28d35bcf928`
- 可莉 Loon 资源：`https://hub.kelee.one/`
- blackmatrix7 规则：`https://github.com/blackmatrix7/ios_rule_script`
- luestr 图标与规则资源：`https://github.com/luestr/IconResource`
- Qure 图标：`https://github.com/Koolson/Qure`
